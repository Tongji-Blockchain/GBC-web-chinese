<?php 
$pageTitle='技术委员会'; //THIS MUST BE SET FOR EACH PAGE
$stylesheet='css/organizers.css';
require 'header-left-bar.php'; //THIS MUST BE INCLUDED FOR EACH PAGE
?>

<div style="margin-top:5%; margin-bottom: 5%" class="flex-column">

	<div class="organizer-group-title">技术委员会联合主席</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/elisa.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Elisa Bertino</div>
			<!-- <div class="role bold">IEEE会士, ACM会士</div>-->
			<div class="affiliation">普渡大学</div>
		</div>
	</div>
	
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/cuishuguang.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">崔曙光</div>
			<!-- <div class="role bold">IEEE会士</div>-->
			<div class="affiliation">香港中文大学（深圳）</div>
		</div>
	</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/renkui.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">任奎</div>
			<!-- <div class="role bold">IEEE会士, ACM会士</div>-->
			<div class="affiliation">浙江大学</div>
		</div>
	</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/chonggangwang.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Chonggang Wang</div>
			<!-- <div class="role bold">IEEE会士，IEEE物联网期刊创刊主编，IEEE Network期刊主编</div>-->
			<!-- <div class="affiliation">InterDigital公司，美国</div>-->
			<!-- <div class="role bold">IEEE Fellow, ACM Fellow</div> -->
			<div class="affiliation">InterDigital</div>
		</div>
	</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/leizhang.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Lei Zhang</div>
			<!-- <div class="role bold">IEEE物联网期刊、IEEE无线通信快报和数字通信与网络的副主编</div>-->
			<div class="affiliation">格拉斯哥大学</div>
		</div>
	</div>
	<div class="spacer"></div>

	<div class="organizer-group-title">Track联合主席</div>

	<div class="organizer-track-title">主题 1: 区块链共识、性能和可扩展性</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">邓小铁</div>
			<div class="role bold">IEEE 会士, ACM 会士</div>
			<div class="affiliation">北京大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">段斯斯</div>
			<div class="role bold"></div>
			<div class="affiliation">清华大学</div>
		</div>
	</div>

	<div class="organizer-track-title">主题 2: 区块链技术的创新应用</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">李颉</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">上海交通大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Ashish Kundu</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">Cisco</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">肖江</div>
			<div class="role bold"></div>
			<div class="affiliation">华中科技大学</div>
		</div>
	</div>


	<div class="organizer-track-title">主题 3: 区块链安全(Block4Sec & Sec4Block)</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">王骞</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">武汉大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">张殷乾</div>
			<div class="role bold"></div>
			<div class="affiliation">南方科技大学</div>
		</div>
	</div>


	<div class="organizer-track-title">主题 4: 用于通信和网络基础设施的区块链、DePIN</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">郭嵩</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">香港科技大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">闫峥</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">西安电子科技大学</div>
		</div>
	</div>


	<div class="organizer-track-title">主题 5: 区块链赋能数据要素和人工智能</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">肖斌</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">香港理工大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">黄华威</div>
			<div class="role bold"></div>
			<div class="affiliation">中山大学</div>
		</div>
	</div>


	<div class="organizer-track-title">主题 6: 区块链、Web3.0和元宇宙生态系统</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">成秀珍</div>
			<div class="role bold">IEEE 会士</div>
			<div class="affiliation">山东大学</div>
		</div>
	</div>
	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">王伟</div>
			<div class="role bold"></div>
			<div class="affiliation">北京交通大学</div>
		</div>
	</div>
	
	敬请期待！
	<div class="spacer"></div>

	<!-- <div class="organizer-group-title">Vice TPC Chair & Coordinator</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Hao Xu</div>
			<div class="role bold"></div>
			<div class="affiliation">Tongji University, China</div>
		</div>
	</div>

	<div class="spacer"></div>

	<div class="organizer-group-title">Workshop Chair</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Bin Cao</div>
			<div class="role bold"></div>
			<div class="affiliation"></div>
		</div>
	</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Butian Huang</div>
			<div class="role bold"></div>
			<div class="affiliation"></div>
		</div>
	</div>

	<div class="spacer"></div>
	<div class="organizer-group-title">Panel Chair</div>

	<div class="flex flex-auto person-container">
		<div class="pic headshot round" style="background:url(images/organizers/.png);background-size:100px 100px;"></div>
		<div class="info-container">
			<div class="name">Fei Liu</div>
			<div class="role bold"></div>
			<div class="affiliation"></div>
		</div>
	</div>
	<div class="spacer"></div> -->

<div class="text">

	<div style="">
		<a style="color:blue;font-size: 20px" href="organizers.php">
			<div style="padding-top:6%;float:left">
				大会组委会
			</div>
		</a>
	</div> 

	<div style="clear: both;margin-top:20px">
		<a style="color:blue;font-size: 20px;" href="TPC.php">
			<div style="padding-top:6%;float:left">
			技术委员会
			</div>
		</a>
	</div>


	<div style="clear: both;margin-top:20px">
		<a style="color:blue;font-size: 20px;" href="IFEC.php">
			<div style="padding-top:6%;float:left">
				行业论坛与展览委员会
			</div>
		</a>
	</div>

	<div style="clear: both;margin-top:20px">
		<a style="color:blue;font-size: 20px;" href="operationC.php">
			<div style="padding-top:6%;float:left">
				执行委员会
			</div>
		</a>
	</div>

	<div style="clear: both;margin-top:20px">
		<a style="color:blue;font-size: 20px;" href="advisoryC.php">
			<div style="padding-top:6%;float:left">
				顾问委员会
			</div>
		</a>
	</div>

</div>




</div>

<?php require 'right-bar-footer.php'; //THE MUST BE INCLUDED FOR EACH PAGE ?>